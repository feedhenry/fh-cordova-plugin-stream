...
@synthesize vbr;
@synthesize guessContentType;
@synthesize realContentType;
	...
	@synchronized(self)
	{
		...
		if (fileLength > 0 && seekByteOffset > 0)
		{
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"),
				(CFStringRef)[NSString stringWithFormat:@"bytes=%i-%i", seekByteOffset, fileLength - 1]);
			discontinuous = vbr;
		}
	...
	@synchronized(self)
	{
		if (state != AS_STARTING_FILE_THREAD)
		{
			if (state != AS_STOPPING &&
				state != AS_STOPPED)
			{
				NSLog(@"### Not starting audio thread. State code is: %i", state);
			}
	...
	eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (!audioFileStream)
		{
			...
			//
            self.guessContentType = [httpHeaders objectForKey:@"Content-Type"];
			AudioFileTypeID fileTypeHint =
				[AudioStreamer hintForMIMEType:self.guessContentType];
 ...
 */
						if ([[lineItems objectAtIndex:0] caseInsensitiveCompare:@"Content-Type"] == NSOrderedSame)
						{
                            ...
                            self.realContentType = [lineItems objectAtIndex:1];
                            if([self.realContentType caseInsensitiveCompare:self.guessContentType] != NSOrderedSame){
                                AudioFileTypeID fileTypeHint =
                                [AudioStreamer hintForMIMEType:self.realContentType];
